...
class LaravelDirUtil51
{
    ...
    public static function showFolders($returnStringAndNoEcho = false){
        ...
        $appFolderInfo = $appFolderInfo.'routerFile            = '.self::getRouterFile()          ."\n";   // 路由文件
...
//        return compact('packageDir','vendorDir','vendorBaseDir','baseDir');
    ...
    public static function getSeederTemplate(){
        return implode(self::getDirSeperator(),[self::getAppRoot(),'database','seeds','entity_seeder.php']);            // 测试数据种子文件
    }

    public static function getRouterFile($routerFilename = 'routes.php'){
        return implode(self::getDirSeperator(),[self::getAppRoot(),'app','Http',$routerFilename]);          // 路由文件
    }
